...
import { InfoFunctionCall } from "./util"
import { MIRBasicBlock, MIRJumpCond, MIROp, MIROpTag, MIRVarStore } from "../compiler/mir_ops";
...
function opToAssignment(op: MIROp): [VarTerm, TermExpr] {
    ...
    let temporal = [new VarTerm("temporal", intType), new ConstTerm("0", intType)] as [VarTerm, TermExpr];
    switch (op.tag) {
    ...
    function traverse(block: MIRBasicBlock): ExprExpr {
        ...
        switch (currentFlow.succs.size) {
            case 0: {
                ...
                currentBlockFormula.ops.reverse();
                let lastOp = currentBlockFormula.ops[0] as MIRVarStore; 
                console.assert(lastOp != undefined);
                // CONTINUE WORKING HERE
                return new ReturnExpr(new ConstTerm("0", intType));
            }
            case 1: {
                let successorLabel = currentFlow.succs.values().next().value;
                return new AssignmentExpr(new VarTerm("temporal", intType), new ConstTerm("1", intType),
            ...
            case 2: {
                ...
                let condition = new FuncTerm("op_Equality", [new VarTerm(regName, boolType), new ConstTerm("true", boolType)], boolType);
