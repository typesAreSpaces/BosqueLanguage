...
let nSMain__main  = 
 ...
 let node2 = __tmp_21 in 
 let n = BNone in 
 let __tmp_28 = (BTuple 2 (SCons (BInt 10) 1 (SCons (BInt 30) 0 SNil))) in 
 let xTuple2 = __tmp_28 in 
 let __tmp_31 = (BTuple 2 (SCons (BInt 10) 1 (SCons xTuple2 0 SNil))) in 
 let xTuple2_1 = __tmp_31 in 
 let __tmp_36 = (BTuple 2 (SCons (BInt 10) 1 (SCons (BInt 3) 0 SNil))) in 
 let __tmp_34 = (BTuple 2 (SCons (BInt 10) 1 (SCons __tmp_36 0 SNil))) in 
 let xTuple2_2 = __tmp_34 in 
 let _ = assert_norm(subtypeOf bTupleType_2BIntType_bTupleType_2BIntType_BIntTypefalsefalse (getType xTuple2_1)) in let __tmp_41 = (nthTuple 1 2 xTuple2_1) in 
 let xTuple_second = __tmp_41 in 
 let _ = assert_norm(subtypeOf bTupleType_2BIntType_bTupleType_2BIntType_BIntTypefalsefalse (getType xTuple2_2)) in let __tmp_44 = (nthTuple 1 2 xTuple2_2) in 
 let xTuple_second_ = __tmp_44 in 
 let _ = assert_norm(subtypeOf bTupleType_2BIntType_BIntTypefalse (getType xTuple_second)) in
  let __tmp_45 = (nSMain__max3 xTuple_second) in 
 let x_max_tuple = __tmp_45 in 
 let _ = assert_norm(subtypeOf bTupleType_2BIntType_BIntTypefalse (getType xTuple_second_)) in
  let __tmp_47 = (nSMain__max3 xTuple_second_) in 
 let x_max_tuple_ = __tmp_47 in 
 let __tmp_51 = (BTuple 2 (SCons (BInt 1) 1 (SCons (BInt 2) 0 SNil))) in 
 let __tmp_54 = (BTuple 2 (SCons (BInt 3) 1 (SCons (BInt 4) 0 SNil))) in 
 let __tmp_50 = (BTuple 3 (SCons __tmp_51 2 (SCons __tmp_54 1 (SCons (BInt 5) 0 SNil)))) in 
 let _ = assert_norm(subtypeOf bTupleType_3bTupleType_2BIntType_BIntTypefalse_bTupleType_2BIntType_BIntTypefalse_BIntTypefalse (getType __tmp_50)) in let __tmp_58 = (nthTuple 2 3 __tmp_50) in 
 let proj_tuple = __tmp_58 in 
 let __tmp_72 = (BTuple 2 (SCons (BInt 1) 1 (SCons (BInt 1) 0 SNil))) in 
 let __tmp_59 = (BTuple 16 (SCons (BInt 1) 15 (SCons (BInt 1) 14 (SCons (BInt 1) 13 (SCons (BInt 1) 12 (SCons (BInt 1) 11 (SCons (BInt 1) 10 (SCons (BInt 1) 9 (SCons (BInt 1) 8 (SCons (BInt 1) 7 (SCons (BInt 1) 6 (SCons (BInt 1) 5 (SCons (BInt 1) 4 (SCons __tmp_72 3 (SCons (BTypedString "hello" BAnyType) 2 (SCons (BBool false) 1 (SCons (BBool true) 0 SNil))))))))))))))))) in 
 let x2 = __tmp_59 in 
 let y = (BInt 20) in 
 let _ = assert_norm(subtypeOf BIntType (getType y)) in
  let __tmp_79 = (nSMain__identityUnion y) in 
 let y2 = __tmp_79 in 
 let _ = assert_norm(subtypeOf bTupleType_2BIntType_BIntTypefalse (getType xTuple2)) in let __tmp_84 = (nthTuple 0 2 xTuple2) in 
 let _ = assert_norm(subtypeOf BIntType (getType __tmp_84)) in
  let __tmp_81 = (nSMain__max __tmp_84 y) in 
 let z = __tmp_81 in 
 let _ = assert_norm(subtypeOf BIntType (getType z)) in
  let __tmp_86 = (nSMain__max z y) in 
 let z_max_func_repeated = __tmp_86 in 
 let _ = assert_norm(subtypeOf BIntType (getType (BInt 3))) in
 let _ = assert_norm(subtypeOf BBoolType (getType (BBool false))) in
  let __tmp_89 = (nSMain__maxx (BBool false) (BInt 3)) in 
 let z4 = __tmp_89 in 
 let __tmp_92 = (BTuple 2 (SCons xTuple2 1 (SCons y 0 SNil))) in 
 let zTuple2 = __tmp_92 in 
 let __tmp_96 = (BTuple 2 (SCons y 1 (SCons (BBool false) 0 SNil))) in 
 let _ = assert_norm(subtypeOf bTupleType_2BIntType_BBoolTypefalse (getType __tmp_96)) in
  let __tmp_95 = (nSMain__identityTupleNoneable __tmp_96) in 
 let z5 = __tmp_95 in 
 let __ir_ret__ = z in 
 let _return_ = __ir_ret__ in 
 _return_
