...
function printBosqueTermsFST(fstar_files_directory, user_defined_types_map) {
  ...
  // No support for Regex\n\
    ...
    user_defined_types_map.forEach((value, index) => {
        if (!index.includes("NSCore")) {
            FS.writeSync(fd, "| B" + util_1.sanitizeName(index) + ": "
  ...
  let rec getType x = match x with\n\
    ...
    user_defined_types_map.forEach((value, index) => {
        if (!index.includes("NSCore")) {
            FS.writeSync(fd, "| B" + util_1.sanitizeName(index) + (" _").repeat(value.fields.length)
                + " -> B" + util_1.sanitizeName(index) + "Type\n");
        }
    });
  ...
  op_eqTerm_Seq n x y = match x with\n\
    ...
    user_defined_types_map.forEach((value, index) => {
        if (!index.includes("NSCore")) {
            const name = util_1.sanitizeName(index);
...
exports.printBosqueTermsFST = printBosqueTermsFST;
//# sourceMappingURL=bosqueTermsFST.js.map