...
class MIRConstantDecl {
    ...
    static jparse(jobj) {
        return new MIRConstantDecl(jobj.enclosingDecl, jobj.cname, jobj.key, jparsepragmas(jobj.pragmas), jparsesinfo(jobj.sinfo), jobj.file, jobj.declaredType, mir_ops_1.MIRBody.jparse(jobj.value));
    }
}
...
class MIRInvokeDecl {
    ...
    static jparse(jobj) {
        if (jobj.body) {
            return new MIRInvokeBodyDecl(jobj.enclosingDecl, jobj.iname, jobj.key, jobj.attributes, jobj.recursive, jparsepragmas(jobj.pragmas), jparsesinfo(jobj.sinfo), jobj.file, jobj.params.map((p) => MIRFunctionParameter.jparse(p)), jobj.resultType, jobj.preconditions.map((pre) => [mir_ops_1.MIRBody.jparse(pre[0]), pre[1]]), jobj.postconditions.map((post) => mir_ops_1.MIRBody.jparse(post)), mir_ops_1.MIRBody.jparse(jobj.body));
        }
        else {
            let binds = new Map();
            ...
            return new MIRInvokePrimitiveDecl(jobj.enclosingDecl, jobj.iname, jobj.key, jobj.attributes, jobj.recursive, jparsepragmas(jobj.pragmas), jparsesinfo(jobj.sinfo), jobj.file, binds, jobj.params.map((p) => MIRFunctionParameter.jparse(p)), jobj.resultType, jobj.preconditions.map((pre) => [mir_ops_1.MIRBody.jparse(pre[0]), pre[1]]), jobj.postconditions.map((post) => mir_ops_1.MIRBody.jparse(post)), jobj.implkey, pcodes);
        }
    }
}
...
class MIRInvokePrimitiveDecl extends MIRInvokeDecl {
    constructor(enclosingDecl, iname, key, attributes, recursive, pragmas, sinfo, srcFile, binds, params, resultType, preconds, postconds, implkey, pcodes) {
        super(enclosingDecl, iname, key, attributes, recursive, pragmas, sinfo, srcFile, params, resultType, preconds, postconds);
...
class MIRFieldDecl {
    ...
    static jparse(jobj) {
        return new MIRFieldDecl(jobj.enclosingDecl, jobj.attributes, jobj.fname, jparsesinfo(jobj.sinfo), jobj.file, jobj.fkey, jparsepragmas(jobj.pragmas), jobj.name, jobj.declaredType, jobj.value ? mir_ops_1.MIRBody.jparse(jobj.value) : undefined);
    }
}
...
class MIROOTypeDecl {
    ...
    static jparse(jobj) {
        ...
        if (jobj.isentity) {
            const entity = new MIREntityTypeDecl(jobj.ooname, jparsesinfo(jobj.sinfo), jobj.file, jobj.tkey, jobj.attributes, jparsepragmas(jobj.pragmas), jobj.ns, jobj.name, terms, jobj.provides, jobj.invariants.map((i) => mir_ops_1.MIRBody.jparse(i)), jobj.fields.map((f) => MIRFieldDecl.jparse(f)), jobj.isCollectionEntityType, jobj.isMapEntityType, jobj.isEnum, jobj.isKey);
            jobj.vcallMap.forEach((vc) => entity.vcallMap.set(vc[0], vc[1]));
            return entity;
        }
        else {
            const concept = new MIRConceptTypeDecl(jobj.ooname, jparsesinfo(jobj.sinfo), jobj.file, jobj.tkey, jobj.attributes, jparsepragmas(jobj.pragmas), jobj.ns, jobj.name, terms, jobj.provides, jobj.invariants.map((i) => mir_ops_1.MIRBody.jparse(i)), jobj.fields.map((f) => MIRFieldDecl.jparse(f)));
...
class MIREntityTypeDecl extends MIROOTypeDecl {
    constructor(ooname, srcInfo, srcFile, tkey, attributes, pragmas, ns, name, terms, provides, invariants, fields, isCollectionEntityType, isMapEntityType, isEnum, isKey) {
        super(ooname, srcInfo, srcFile, tkey, attributes, pragmas, ns, name, terms, provides, invariants, fields);
        this.isEnum = isEnum;
        this.isKey = isKey;
