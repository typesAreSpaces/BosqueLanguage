    ...
    generateBuiltinBody(issafe: boolean, idecl: MIRInvokePrimitiveDecl, params: string[]): SMTExp {
        ...
        switch (idecl.implkey) {
            ...
            default: {
                ...
                const parametric_type_instances = [...idecl.binds.values()].map(ty => this.typegen.mangleStringForSMT(ty)).join("$");
                const fname = `@${idecl.implkey}` + (pctag.length !== 0 ? `$${pctag}` : "") + parametric_type_instances;
